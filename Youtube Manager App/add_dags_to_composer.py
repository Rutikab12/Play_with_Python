...
def upload_dags_to_composer(dags_directory: str, test_bucket_name: str, dags_bucket_subfolder : str) -> None:
    ...
    if len(dags) > 0:
        ...
        for dag in dags:
            dag_path = dag.replace(f"{temp_dir}/", f"{dags_directory}/")
            dag = dag.replace(f"{temp_dir}/", f"dags/{dags_bucket_subfolder}/")
        
        
            # Upload to test bucket
            blob = test_bucket.blob(dag)
            blob.upload_from_filename(dag_path)
            print(f"File {dag} uploaded to {test_bucket_name}/{dag}/.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_dags_bucket",
        help="Name of the DAGs bucket of your Test Composer environment without the gs:// prefix",
    )

    parser.add_argument(
        "--dags_bucket_subfolder",
        help="Name of the DAGs bucket subfolder of the LM",
    )

    args = parser.parse_args()

    upload_dags_to_composer(args.dags_directory,args.test_dags_bucket, args.dags_bucket_subfolder)
